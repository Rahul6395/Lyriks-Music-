...
const TopChartCard = ({song,i,isPlaying,handlePauseClick,handlePlayClick,activeSong})=>(
 ...
 <img src={song?.images?.coverart} alt={song?.title} className=" rounded-lg" style={{width:"3.5rem",height:"3.5rem"}}/>
 <div className="flex-1 flex flex-col justify-center mx-3">
 <Link  to={`/songs/${song.key}`}>
  <p className="text-base font-bold text-white truncate w-60">{song?.title}</p>
 </Link>
 <Link  to={`/artists/${song?.artists[0].adamid}`}>
  <p className="text-base text-gray-300 mt-0 truncate w-60">{song?.subtitle}</p>
 </Link>
 </div>
   </div>
...
const topPlays = data?.tracks?.slice(0,5);
  ...
  return(
  <div ref={divRef} className="xl:ml-6 ml-0 xl:mb-0 mb-6 flex-1 xl:max-w-[430px] max-w-full flex-col">
