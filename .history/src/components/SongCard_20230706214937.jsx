...
const SongCard = ({isPlaying,activeSong,data,song,i}) => {
  ...
  <p className="text-sm truncate text-gray-300 mt-1">
    <Link to={song.trackMetadata.artists?`/artists/${song.trackMetadata.artists[0]?.spotify}`:`/top-artists`}>{song.trackMetadata.artists[0]?.name}{song.trackMetadata.artists[1]?.name}{song.trackMetadata.artists[2]?.name}
